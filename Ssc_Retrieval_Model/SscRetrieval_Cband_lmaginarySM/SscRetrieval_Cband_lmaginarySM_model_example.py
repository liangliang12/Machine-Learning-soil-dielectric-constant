...
# moisture:  Soil moisture content(0-0.4)
...
# Salinity: the predicted Soil salinity content

############################################################################################################
# Multi-Sample Batch Prediction for Soil Properties
##Load the saved model  
# this is an example about soil moisture content prediction model
# model_input_path is the address of the data storage folder
model_input_path = r'...\SscRetrieval_Cband_ImaginarySM.plk'
...
X = data[['imaginary','moisture', 'frequency','Sandy', 'Clay', 'rou_s', 'rou_b']]
...
X = {'imaginary':[9.9],'moisture':[0.1], 'frequency':[5.5],'Sandy': [0.46], 'Clay':[0.097], 'rou_s':[2.65], 'rou_b':[1.35]}
if (X['frequency'][0] < 5) or ((X['frequency'][0] > 6)):
    print('This model is not applicable within this frequency range.')
if (X['moisture'][0] < 0) or ((X['moisture'][0] > 0.5)):
    print('This model is not applicable within this moisture content range.')
X = pd.DataFrame(X)
